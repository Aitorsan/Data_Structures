...
namespace asf {
	...
	template <typename T>
	vector<T>::vector(const std::initializer_list<T>& l): m_size(l.size()),m_capacity(m_size*INC_CAPACITY_FACTOR), m_container(new T[m_capacity]),end_pointer(nullptr){
	...
	//Copy constructor
	template <typename T>
	vector<T>::vector(const vector<T>& rhs) :m_size{rhs.m_size }, m_capacity{ rhs.m_capacity }, m_container {new T[m_capacity] }, end_pointer{ nullptr } {
	...
	//move constructor
	template<typename T>
	vector<T>::vector( vector<T>&& rhs):m_size(rhs.m_size),m_capacity(rhs.m_capacity),m_container(rhs.m_container),end_pointer(rhs.end_pointer){
	...
	*************************************/
	...
	inline void vector<T>::push_back(T element) {
		...
		// if the vector is empty, that means we just clean the array or we have transfered the ownership
			...
			//because the array is empty end_pointer has to point to the first free space on the array that fur sure is the first index 0
